...
async def chat(request: ChatRequest, user: AuthUser = Depends(get_current_user)):
    ...
    try:
        ...
        if request.mode == ChatMode.GENERAL:
            ...
            # Combine session history and RAG doc context
        ...
        raise                          # ← FIX 1: let 400s pass through untouched
...
async def solve_image_question(
    ...
    subject: str | None = Form(None),   # ← FIX 2: Form() so multipart data is read correctly
    mode: str | None = Form("steps"),   # ← FIX 2: Form() so multipart data is read correctly
