...
def _make_app_with_db(mock_db):
    from login.app.password_reset_routes import password_reset_router
...
class TestForgotPassword:
    ...
    @patch("login.app.password_reset_routes.send_reset_password_email")
    @patch("login.app.password_reset_routes.hash_token", return_value=TOKEN_HASH)
    @patch("login.app.password_reset_routes.generate_secure_token", return_value=RAW_TOKEN)
    def test_returns_200_when_user_exists(self, *_):
        with patch("login.app.password_reset_routes.get_user_by_email", new=AsyncMock(return_value=_make_user())):
    ...
    def test_returns_200_when_user_does_not_exist(self):
        with patch("login.app.password_reset_routes.get_user_by_email", new=AsyncMock(return_value=None)):
            assert self.client.post(self.url, json=self.PAYLOAD).status_code == 200
    ...
    def test_email_not_sent_when_user_not_found(self):
        with patch("login.app.password_reset_routes.get_user_by_email", new=AsyncMock(return_value=None)), \
             patch("login.app.password_reset_routes.send_reset_password_email") as mock_send:
            self.client.post(self.url, json=self.PAYLOAD)
    ...
    def test_token_hash_stored_not_raw_token(self, *_):
        ...
        with patch("login.app.password_reset_routes.get_user_by_email", new=AsyncMock(return_value=_make_user())), \
             patch("login.app.password_reset_routes.PasswordResetToken") as MockPRT:
    ...
    def test_returns_200_on_unexpected_exception(self):
        ...
        with patch("login.app.password_reset_routes.get_user_by_email",
...
class TestResetPassword:
    ...
    @patch("login.app.password_reset_routes.hash_token", return_value=TOKEN_HASH)
    @patch("login.app.password_reset_routes.hash_password", return_value="$2b$12$newhash")
