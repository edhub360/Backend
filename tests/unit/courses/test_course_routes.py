...
class TestListCoursesEndpoint:
    ...
    def test_returns_200_with_courses(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(1, [make_course_row()]))):
    ...
    def test_short_description_truncated_to_200(self):
        ...
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(1, [course]))):
            data = self.client.get("/courses/").json()
    ...
    def test_last_item_rotated_to_first(self):
        ...
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(3, courses))):
            data = self.client.get("/courses/").json()
    ...
    def test_single_item_rotation_is_stable(self):
        course = make_course_row(course_id=UUID_ONLY)
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(1, [course]))):
            data = self.client.get("/courses/").json()
        assert data["items"][0]["course_id"] == UUID_ONLY

    def test_empty_items_list(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(0, []))):
            data = self.client.get("/courses/").json()
        assert data["items"] == []
        assert data["total"] == 0

    def test_pagination_meta_reflects_query_params(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(50, []))):
    ...
    def test_search_query_passed_to_crud(self):
        ...
        with patch("courses.app.routes.courses.list_courses", new=mock_list):
            self.client.get("/courses/?q=python")
        mock_list.assert_called_once()
        assert mock_list.call_args[0][1] == "python"
...
class TestGetCourseEndpoint:
    ...
    def test_returns_200_when_found(self):
        with patch("courses.app.routes.courses.get_course", new=AsyncMock(return_value=make_course_row())):
            response = self.client.get(f"/courses/{COURSE_UUID_1}")
        assert response.status_code == 200

    def test_returns_404_when_not_found(self):
        with patch("courses.app.routes.courses.get_course", new=AsyncMock(return_value=None)):
    ...
    def test_created_at_is_isoformat_string(self):
        with patch("courses.app.routes.courses.get_course", new=AsyncMock(return_value=make_course_row(created_at=datetime(2024, 6, 1, 12, 0, 0)))):
            data = self.client.get(f"/courses/{COURSE_UUID_1}").json()
        assert data["created_at"] == "2024-06-01T12:00:00"

    def test_course_id_passed_to_crud(self):
        mock_get = AsyncMock(return_value=None)
        with patch("courses.app.routes.courses.get_course", new=mock_get):
            self.client.get(f"/courses/{UUID_SPECIFIC}")
        mock_get.assert_called_once()

    def test_full_desc_returned_not_truncated(self):
        with patch("courses.app.routes.courses.get_course", new=AsyncMock(return_value=make_course_row(course_desc="D" * 500))):
            data = self.client.get(f"/courses/{COURSE_UUID_1}").json()
        assert len(data["course_desc"]) == 500

    def test_course_credit_value(self):
        with patch("courses.app.routes.courses.get_course", new=AsyncMock(return_value=make_course_row(course_credit=5))):
            data = self.client.get(f"/courses/{COURSE_UUID_1}").json()
...
class TestFeaturedCoursesEndpoint:
    ...
    def test_returns_200(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(3, [make_course_row() for _ in range(3)]))):
            response = self.client.get("/courses/featured")
        assert response.status_code == 200

    def test_page_is_always_1(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(2, [make_course_row(), make_course_row()]))):
    ...
    def test_crud_called_with_page_1(self):
        ...
        mock_list.assert_called_once()
        assert mock_list.call_args[0][2] == 1
    ...
    def test_short_description_truncated(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(1, [make_course_row(course_desc="Y" * 300)]))):
            data = self.client.get("/courses/featured").json()
        assert len(data["items"][0]["short_description"]) == 200

    def test_none_desc_becomes_empty_string(self):
        with patch("courses.app.routes.courses.list_courses", new=AsyncMock(return_value=(1, [make_course_row(course_desc=None)]))):
            data = self.client.get("/courses/featured").json()
        assert data["items"][0]["short_description"] == ""
