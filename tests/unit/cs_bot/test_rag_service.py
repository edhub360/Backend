...
class TestGetLlm:
    ...
    def test_returns_llm_instance(self):
        mock_llm = MagicMock()
        with patch("cs_bot.app.services.rag_service.ChatGoogleGenerativeAI", return_value=mock_llm), \
             patch("cs_bot.app.services.rag_service.settings") as s:
    ...
    def test_uses_correct_model(self):
        with patch("cs_bot.app.services.rag_service.ChatGoogleGenerativeAI") as mock_cls, \
             patch("cs_bot.app.services.rag_service.settings") as s:
            s.CHAT_MODEL = "gemini-2.5-flash"
            s.GEMINI_API_KEY = "fake-key"
            from cs_bot.app.services.rag_service import get_llm
...
class TestGenerateReply:
    ...
    async def test_returns_reply_string(self, mock_vector_store):
        ...
        mock_retriever.ainvoke = AsyncMock(return_value=[
            MagicMock(page_content="Edhub info", metadata={"source": "https://edhub.com"})
        ])
        ...
        with patch("cs_bot.app.services.rag_service.get_vector_store", return_value=mock_vector_store), \
             patch("cs_bot.app.services.rag_service.get_llm", return_value=mock_llm), \
             patch("cs_bot.app.services.rag_service.ChatPromptTemplate.from_messages", return_value=mock_prompt), \
             patch("cs_bot.app.services.rag_service.StrOutputParser", return_value=mock_parser):
            from cs_bot.app.services.rag_service import generate_reply
            reply, _ = await generate_reply("What is Edhub?", [])
